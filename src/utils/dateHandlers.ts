...
export const getPreviousAndNextWeek = (
  ...
  console.info(lastDayOfPrevMonth);
  const previousWeek = [];
  ...
  for (let i = 0; i < 7; i++) {
    const result = sub(lastDayOfPrevMonth, { days: i });

    previousWeek.push(result);
  }
  const monday = previousWeek.findIndex((item) => {
    return item.getDay() === 1;
  });

  console.info(monday);
  return {
    previousWeek: previousWeek.reverse(),
    nextWeek: nextWeek,
  };
};
