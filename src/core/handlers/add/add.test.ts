...
describe("fn should return prevDate", () => {
	test("Should return date with added dates", () => {
		...
		for (const test of [
			...
			expect(formatDate(add({ date: date, count: test.count, type: "day" }))).toBe(test.expect);
		}
	});
	...
	test("Should return date with added months", () => {
		...
		for (const test of [
			...
			expect(formatDate(add({ date: mockDateObject, count: test.count, type: "month" }))).toBe(test.expect);
		}
	});
	...
	test("Should return date with added years", () => {
		...
		for (const test of [
			...
			expect(formatDate(add({ date: mockDateObject, count: test.count, type: "year" }))).toBe(test.expect);
		}
	});
	...
	test("Should return next week to passed date", () => {
		...
		for (const test of [
			...
			expect(formatDate(add({ date: mockDateObject, count: test.count, type: "week" }))).toBe(test.expect);
		}
	});
});
