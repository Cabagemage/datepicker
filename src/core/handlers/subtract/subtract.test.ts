...
describe("fn should return prevDate", () => {
  test("Should return prev week start", () => {
    ...
    for (const test of [
      ...
      expect(
        formatDate(subtract({ date: date, count: test.count, type: "day" }))
      ).toBe(test.expect);
    }
  });
  ...
  test("Should return prev month start", () => {
    ...
    for (const test of [
      ...
      expect(
        formatDate(
          subtract({ date: mockDateObject, count: test.count, type: "month" })
        )
      ).toBe(test.expect);
    }
  });
  ...
  test("Should return substracted years date", () => {
    ...
    for (const test of [
      ...
      expect(
        formatDate(
          subtract({ date: mockDateObject, count: test.count, type: "year" })
        )
      ).toBe(test.expect);
    }
  });
  ...
  test("Should return prev week", () => {
    ...
    for (const test of [
      ...
      expect(
        formatDate(
          subtract({ date: mockDateObject, count: test.count, type: "week" })
        )
      ).toBe(test.expect);
    }
  });
