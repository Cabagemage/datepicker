...
export const getMonthCalendarViewDates: GetMonthCalendarViewDates = ({
  ...
  const fiirstWeekOfMonth = getFirstWeekOfMonth({    initialDate,
    month,
    year,})
  const lastWeekOfMonth = getLastWeekOfMonth({    initialDate,
    month,
    year,})
  ...
  const temporaryArray = fiirstWeekOfMonth.concat(currentMonth).concat(lastWeekOfMonth);
